...
class SafedialTest < Minitest::Test
  ...
  def test_start_position_is_correct
    assert_equal(50, @self_dial.position)
  end
  ...
  def test_safe_dial_detects_zero_during_rotation
    ...
    @self_dial.dial
    assert_equal(1, @self_dial.zeros)
  end
  def test_safe_detects_zero_during_multiple_rotations
    @self_dial.instructions = ["L68", "L30", "R48", "L5", "R60", "L55", "L1", "L99", "R14", "L82"]
    @self_dial.dial
    assert_equal(3, @self_dial.zero_count)
  end
